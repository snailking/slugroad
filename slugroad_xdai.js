var contractAddress="0xc726B922d60680CaC9Df99c1D5e1AB9138752265"; //xDai v1
...
var timeLaunch = 1551620583;
var launchBlock = 2457813;
var blockSpan = 5; //5 seconds blocks on xDai Chain
var startBlock = 0;
var ranLog = false;
...
function updateLocalTimer(){
	...
	if(s_hyperState == 1 && a_driver != no_driver){
		...
		doc_gameState.innerHTML = 'Ether drained: ' + parseFloat(l_etherDrained).toFixed(6) + ' xDai';
...
function setDriveBuy(){
	document.getElementById('fieldBuy').value = parseFloat(a_buyCost * 200 + 0.0001).toFixed(5);
}
					...
					if(i < result.length){ //make sure there's enough events
						if(checkHash(result[i].transactionHash, result[i].event) != 0) {
							...
							if(result[i].event == "WithdrewBalance"){
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " withdrew " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai to their wallet.";								
							} else if(result[i].event == "BoughtSlug"){
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " bought " + result[i].args.slug + " slugs for " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai.";		
							} else if(result[i].event == "SkippedAhead"){
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " skipped in time, and got " + result[i].args.slug + " slugs for " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai!";
							} else if(result[i].event == "TradedMile"){
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " traded " + parseInt((result[i].args.mile) * 6000) + " miles. Their reward: " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai.";			
								...
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " took the wheel! They're next in line to win " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai.";
								...
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " jumped out of the car, snatching " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai on their way out!";
							} else if(result[i].event == "TimeWarped"){
								eventlogdoc.innerHTML += "<br>[" + datetext + "] TIME WARP! Loop " + parseInt(result[i].args.loop - 1) + " is over. " + formatEthAdr(result[i].args.player) + " drove the car to the future and won " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai. Loop " + result[i].args.loop + " starts!";							
								...
								eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result[i].args.player) + " makes a generous " + formatEthValue2(web3.fromWei(result[i].args.eth,'ether')) + " xDai donation to the slugBank.";
							}
...
withdrewbalanceEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " withdrew " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai to their wallet.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
boughtslugEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " bought " + result.args.slug + " slugs for " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});	
...
skippedaheadEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " skipped in time, and got " + result.args.slug + " slugs for " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});	
...
tradedmileEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " traded " + parseInt((result.args.mile) * 6000) + " miles. Their reward: " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});	
...
tookwheelEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " took the wheel! They're next in line to win " + a_loopChest + " xDai.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
jumpedoutEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " jumped out of the car, snatching " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai on their way out!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});					
...
timejumpedEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] TIME WARP! Loop " + parseInt(result.args.loop - 1) + " is over. " + formatEthAdr(result.args.player) + " drove the car to the future and won " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai. Loop " + result.args.loop + " starts!";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
paidthroneEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " paid tribute to the SnailThrone! " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai has been sent.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
...
boostedpotEvent.watch(function(error, result){
    if(!error){
		...
		if(checkHash(result.transactionHash, result.event) != 0) {
			date24();
			eventlogdoc.innerHTML += "<br>[" + datetext + "] " + formatEthAdr(result.args.player) + " makes a generous " + formatEthValue2(web3.fromWei(result.args.eth,'ether')) + " xDai donation to the slugBank.";
			logboxscroll.scrollTop = logboxscroll.scrollHeight;
		}
	}
});
